...
@router.get("/", response_model=List[InformacionContratistaResponse])
async def get_informacion_contratista(
    id_obra: int = Query(..., description="ID de la obra"),
    db: Session = Depends(get_db)
):
    """Obtener toda la información contratista de una obra"""
    try:
        print(f"🔍 Buscando información contratista para obra: {id_obra}")
        
        info = db.query(InformacionContratista).filter(
            InformacionContratista.id_obra == id_obra
        ).all()
...
async def create_informacion_contratista(
    data: InformacionContratistaCreate,
    db: Session = Depends(get_db)
):
    """Crear nueva información contratista"""
    try:
        print(f"📝 Creando información contratista: {data}")
        ...
        new_info = InformacionContratista(**data_dict)
        db.add(new_info)
        db.commit()
        db.refresh(new_info)
        ...
        response_data = {
            "id": new_info.id,
            "id_tipo_contratista": new_info.id_tipo_contratista,
...
@router.put("/{id}", response_model=InformacionContratistaResponse)
async def update_informacion_contratista(
    id: int,
    ...
    try:
        info = db.query(InformacionContratista).filter(InformacionContratista.id == id).first()
        if not info:
...
async def delete_informacion_contratista(
    id: int,
    db: Session = Depends(get_db)
):
