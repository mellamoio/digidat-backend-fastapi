...
from schema.role_schema import RoleSchema, RoleCreateSchema
...
@router.get('/api/v1/role',status_code=HTTP_200_OK, response_model=List[RoleSchema], tags=['Role'])
def get_roles():
...
@router.post('/api/v1/role', status_code=HTTP_201_CREATED, tags=['Role'])
...
@router.get('/api/v1/role/{id}',status_code=HTTP_200_OK, response_model=RoleSchema, tags=['Role'])
def get_role(id:int):
    with engine.begin() as conn:
...
@router.put('/api/v1/role/{id}', status_code=HTTP_200_OK, response_model=RoleCreateSchema, tags=['Role'])
...
@router.delete('/api/v1/role/{id}', response_model=RoleSchema, status_code=HTTP_200_OK, tags=['Role'])
